function [fitobj, fitcoeffs, fiterrs]= plotseries(data, ix, iy, dataNums )
    if nargin <= 0
    ...
    for iPlot=1:nPlots
        ...
        leg{leglines*(iPlot-1)+1}=data.names{iData};
    ...
    % final
    title(['Dependence of ' data.colNames{dataNums(1)}{iy(1)} ' on ' data.colNames{dataNums(1)}{ix(1)}]);
    xlabel([data.colNames{dataNums(1)}{ix(1)} ' [' data.colUnits{dataNums(1)}{ix(1)} ']']);
    ylabel([data.colNames{dataNums(1)}{iy(1)} ' [' data.colUnits{dataNums(1)}{iy(1)} ']']);
    hold off;
end 