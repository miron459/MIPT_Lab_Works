...
function [fitObjs] = plotsubplot1(dc, ix, iy, ixData, iyData, iSubplot, gridSize)    
    ...
    for iPlot=1:nPlots
        ...
        fitOpts.Exclude=[length(X)-4:length(X)];
        if(iyErr>0) 
            YErr=dc.data{iyDatum}(:,iyErr);
            fitOpts.Weights = 1./YErr.^2;
        ...
        newleglines=generateLegendLines(dc, ix(iPlot), ixDatum, X,  iy(iPlot), iyDatum, Y, YErr, fitObj);
    ...
    xUnits="[" + dc.colUnits{ixDatum}(ix(1)) +"]"; if(xUnits=="[]"); xUnits=""; end
    yUnits="[" + dc.colUnits{iyDatum}(iy(1)) +"]"; if(yUnits=="[]"); yUnits=""; end
    xlabel([dc.colNames{ixDatum}{ix(1)} ' ' xUnits{:}]);
    ylabel([dc.colNames{iyDatum}{iy(1)} ' ' yUnits{:}]);
    hold off;
end

%% support functions
function leglines=generateLegendLines(dc, ix, ixDatum, X,  iy, iyDatum, Y, YErr, fitObj)
    format='%5.1f';
    ...
    fitOpts=fitoptions(fitObj);
    X=X(setdiff(1:end, fitOpts.Exclude));
    Y=Y(setdiff(1:end, fitOpts.Exclude));
    YErr=YErr(setdiff(1:end, fitOpts.Exclude));
    
    chi2=sum(((Y-fitObj(X))./YErr).^2);
    pvalue=1-chi2cdf(chi2,length(X)-numcoeffs(fitObj));
    chi2ToStr="\chi^2 = "+num2str(chi2, '%3.1f');
    pvalueToStr="p-value = "+num2str(pvalue*100, '%3.1f')+"%";
    
    fitToStr=char(coeffnames(fitObj))+" = "+fitCoeffsToStr+"±"+fitConfIntsToStr+" " +units;
    leglines=cellstr([plotTitle; fitTitle; fitToStr; chi2ToStr; pvalueToStr]);
end
