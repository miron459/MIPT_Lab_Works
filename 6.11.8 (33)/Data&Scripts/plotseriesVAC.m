function [fitObjs, ix, iy, iData] = ...
        plotseriesVAC(dc, ix, iy, iData, iSubplots, gridSize)
    %% parse arguments
...
function [fitObjs] = plotsubplot1(dc, ix, iy, iData, iSubplot, gridSize)    
    ...
    for iPlot=1:nPlots
        iDatum=iData(iPlot);
        YErr=zeros(max(size(dc.data{iDatum}(:,ix(iPlot)))),1)+0.001;
        ...
        leg(leglines*(iPlot-1)+1)=dc.colNames{iDatum}{iy(iPlot)};
        leg(leglines*(iPlot-1)+2)='linear fit R*I+b:';
        leg(leglines*(iPlot-1)+3)=['R = ' num2str(fitCoeffs(iPlot, 1), format)...
            '±' num2str(abs(fitConfInts(iPlot,1,1)-fitConfInts(iPlot,1,2))/2, formaterr) ' mOhm'];
    ...
    %% final
    iDatum=iData(1);
    title(['Voltage - Current characteristics']);
    xlabel([dc.colNames{iDatum}{ix(1)} ' [' dc.colUnits{iDatum}{ix(1)} ']']);
    ylabel(['Voltage'                  ' [' dc.colUnits{iDatum}{iy(1)} ']']);
    hold off;
end
